...
export const createSiteChromeRenderer = ({ base, site }) => {
  ...
  const replacements = {
    ...
    email: () => `<a href="mailto:${escapeHtml(site.identity.email)}">${escapeHtml(site.identity.email)}</a>`,
    phone: () => `<a href="tel:${escapeHtml(site.identity.phone.href)}">${escapeHtml(site.identity.phone.text)}</a>`,
